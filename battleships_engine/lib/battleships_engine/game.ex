defmodule BattleshipsEngine.Game do
  ...
  def handle_call({:position_ships, player, key, row, col}, _from, state_data) do
    board = player_board(state_data, player)
    with {:ok, rules} <-
            Rules.check(state_data.rules, {:position_ships, player}),
          {:ok, coordinate} <-
            Coordinate.new(row, col),
          {:ok, ship} <-
            Battleship.new(key, coordinate),
          %{} = board <-
            Board.position_ship(board, key, ship)
    do
      state_data
